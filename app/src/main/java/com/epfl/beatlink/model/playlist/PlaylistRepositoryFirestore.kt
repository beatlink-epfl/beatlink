...
class PlaylistRepositoryFirestore(
    ...
    private val auth: FirebaseAuth = FirebaseAuth.getInstance()) : PlaylistRepository {
    ...
    override fun getPlaylists(onSuccess: (List<Playlist>) -> Unit, onFailure: (Exception) -> Unit) {
        db.collection(collectionPath).whereEqualTo("userId", userID).get()
            .addOnSuccessListener { res ->
                val playlistList = res?.documents?.mapNotNull { doc ->
                    documentToPlaylist(doc)
    ...
    override fun updatePlaylist(
        ...
        try {
            db.collection(collectionPath).document(playlist.playlistID)
                .set(playlist)
                .addOnSuccessListener {
                    onSuccess()
                }
