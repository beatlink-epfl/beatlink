...
class SpotifyAuthRepository(private val client: OkHttpClient): MusicServiceAuthRepository {
        ...
        try {
            ...
            if (response.isSuccessful) {
                response.body?.let { responseBody ->
                    ...
                    // Check if there's a new refresh token and save it if it exists
                ...
                emit(Result.failure(Exception("An error occurred retrieving the Spotify access token")))
            }
        } catch (e: IOException) {
    ...
    suspend fun requestAccessToken(
        ...
        if (codeVerifier == null) {
            emit(Result.failure(Exception("Code verifier not found")))
            return@flow
        }
