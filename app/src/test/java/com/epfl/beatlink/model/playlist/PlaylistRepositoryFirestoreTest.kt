...
class PlaylistRepositoryFirestoreTest {
    ...
    @Mock
    private lateinit var mockTask: com.google.android.gms.tasks.Task<QuerySnapshot>
    @Mock
    ...
    @Mock
    private lateinit var mockPlaylistQuerySnapshot: QuerySnapshot
     ...
     */
    @Test
    fun `getPlaylists should retrieve playlists owned by the current user`() {
        ...
        `when`(mockCollectionReference.whereEqualTo("userId", mockFirebaseUser.uid)).thenReturn(mockQuery)
        `when`(mockQuery.get()).thenReturn(Tasks.forResult(mockQuerySnapshot))

        val documents = listOf(mockDocumentSnapshot2)
        `when`(mockQuerySnapshot.documents).thenReturn(documents) // Firestore returns a QuerySnapshot, which contains the list of DocumentSnapshots that match the query.
        `when`(mockDocumentSnapshot2.toObject(Playlist::class.java)).thenReturn(playlist2)

        val onSuccess: (List<Playlist>) -> Unit = mock()
        val onFailure: (Exception) -> Unit = mock()

        playlistRepositoryFirestore.getPlaylists(onSuccess, onFailure)

        verify(onSuccess).invoke(listOf( playlist2))
    ...
    fun `getPlaylists should call onFailure on error`() {
        ...
        `when`(mockFirestore.collection("playlists")).thenReturn(mockCollectionReference)
        `when`(mockCollectionReference.whereEqualTo("userId", "testUserId")).thenReturn(mockQuery)
    ...
    fun updatePlaylistTrackCount_shouldCallFirestoreUpdate() {
        ...
        `when`(mockFirestore.collection("playlists").document("playlistID")).thenReturn(mockDocumentReference)
        `when`(mockDocumentReference.update("nbTracks", 10)).thenReturn(Tasks.forResult(null)) // Simulate success
    ...
    fun updatePlaylistSongs_withFirestoreError() {
        ...
        val exception = Exception("Firestore update error")
        `when`(mockDocumentReference.update("playlistSongs", newSongs)).thenReturn(Tasks.forException(exception))
    ...
    fun updatePlaylistSongs_shouldCallOnSuccessWhenUpdateIsSuccessful() {
        ...
        val playlist = Playlist(playlistID = "playlistID",
            playlistCover = "",
            playlistName = "Test Playlist",
            userId = "testUserId",
        ...
        `when`(mockFirestore.collection("playlists").document(playlist.playlistID)).thenReturn(mockDocumentReference)
        `when`(mockDocumentReference.update("playlistSongs", newSongsList)).thenReturn(Tasks.forResult(null)) // Successful update
    ...
    fun updatePlaylistSongs_shouldCallOnFailureWhenUpdateFails() {
        ...
        `when`(mockFirestore.collection("playlists").document(playlist.playlistID)).thenReturn(mockDocumentReference)
        `when`(mockDocumentReference.update("playlistSongs", newSongsList)).thenReturn(Tasks.forException(Exception("Firestore update error"))) // Simulate failure
    ...
    fun deletePlaylistById_shouldCallOnFailureWhenErrorOccurs() {
        ...
        `when`(mockFirestore.collection("playlists").document(playlistId)).thenReturn(mockDocumentReference)
        `when`(mockDocumentReference.delete()).thenReturn(Tasks.forException(Exception("Firestore delete error"))) // Simulate failure
