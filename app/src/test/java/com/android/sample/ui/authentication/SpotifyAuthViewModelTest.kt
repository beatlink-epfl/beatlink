...
class SpotifyAuthViewModelTest {
	...
	@Test
	fun `authState is Authenticated if access token exists and is valid`() = runTest(testDispatcher) {
	...
	@Test fun `handleAuthorizationResponse sets authState to Authenticated on success`() = runTest(testDispatcher) {
	...
	@Test
	fun `handleAuthorizationResponse sets authState to Idle on failure`() = runTest(testDispatcher) {
		val intent = mock(Intent::class.java)
		val uri = mock(Uri::class.java)
