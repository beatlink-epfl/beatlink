...
Permission for Fine-grained location is ok for beatlink.
...
The mockup is present, however the wireframe is rather small and somplistic. Ideally, the wireframe should be done before the mock-up, therefore the twos should be of rather similar sizes.
The mock-up describes all the core features, they are well defined and detailed. The wireframe is less detailed.
Overall, the mock-up looks great.
...
The login and sign up button are not reachable when rotating the screen.
...
Overall the tests are good and cover enough code logic.
...
The application works as expected, not bugs were found.
...
The team retrospective was not present in the wiki at the time of the deadline.
...
Some tasks are still drafts, with only two persons assigned to some tasks, some tasks are missing time estimate, assignee and priority.
